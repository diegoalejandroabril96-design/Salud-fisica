// Version 11: cardio-first cholesterol-aware routine with short strength finisher.
const DAYS = [
  { day: "Lunes", title: "Cardio + superior", focus: "Lazo por intervalos y fuerza corta de torso", split: "CARDIO 77% / FUERZA 17%", exercises: ["Burpee adaptado", "Remo con pesas", "Press de hombros", "Curl de biceps", "Flexiones inclinadas"] },
  { day: "Martes", title: "Cardio + inferior", focus: "Lazo por intervalos y piernas intensas", split: "CARDIO 77% / FUERZA 17%", exercises: ["Burpee adaptado", "Sentadilla con pesas", "Peso muerto rumano", "Zancada atras", "Elevacion de talones"] },
  { day: "Miercoles", title: "Cardio + core", focus: "Lazo por intervalos, abdomen y empuje", split: "CARDIO 77% / FUERZA 17%", exercises: ["Burpee adaptado", "Remo inclinado", "Flexiones", "Rueda abdominal corta", "Rodilla-codo lateral"] },
  { day: "Jueves", title: "Cardio + inferior", focus: "Lazo por intervalos y potencia controlada", split: "CARDIO 77% / FUERZA 17%", exercises: ["Burpee adaptado", "Sentadilla con pesas", "Peso muerto rumano", "Puente de gluteos", "Rodillas arriba"] },
  { day: "Viernes", title: "Cardio + superior", focus: "Lazo por intervalos, brazos y hombros", split: "CARDIO 77% / FUERZA 17%", exercises: ["Burpee adaptado", "Remo con pesas", "Press de hombros", "Curl de biceps", "Dead bug"] },
  { day: "Sabado", title: "Cardio + mixto", focus: "Lazo por intervalos y cuerpo completo", split: "CARDIO 77% / FUERZA 17%", exercises: ["Burpee adaptado", "Sentadilla sin peso", "Remo inclinado", "Escaladores lentos", "Elevacion de talones"] }
];
...
const INFO = {
  ...
  "Remo inclinado": ["Junta los omoplatos | Manten el torso estable y lleva ambos codos hacia atras con control.", "row"],
  ...
  "Elevacion de talones": ["Sube y baja sobre las puntas | Manten el cuerpo alto, pausa un segundo arriba y baja talones lentamente.", "calf"],
...
const bodyMetrics = JSON.parse(localStorage.getItem("fuerte-body") || '{"weights":[]}');
...
function midPose(a, b) {
  return {
    head: mixPoint(a.head, b.head, .5),
    torso: mixPoint(a.torso, b.torso, .5),
    hip: mixPoint(a.hip, b.hip, .5),
    arms: a.arms.map((limb, i) => mixLimb(limb, b.arms[i], .5)),
    legs: a.legs.map((limb, i) => mixLimb(limb, b.legs[i], .5)),
    cues: ["Movimiento", "Controlado"],
    load: a.load || b.load,
    rope: a.rope || b.rope
  };
}
function exerciseFrame(pose, label, x) {
  return `<g transform="translate(${x} 35) scale(.62)">
    ${figure(pose)}
    <path d="M36 192h248" class="ground"/>
    <text x="155" y="236" text-anchor="middle" class="sequence-label">${label}</text>
  </g>`;
}
...
function art(type, rest = false) {
  ...
  return `<svg viewBox="0 0 680 285" class="coach-svg motion-sequence" aria-label="Secuencia visual del ejercicio">
    ...
    <rect x="12" y="8" width="656" height="265" rx="24" class="panel"/>
    <text x="30" y="34" class="svg-label">MOVIMIENTO GUIADO</text>
    <text x="648" y="34" text-anchor="end" class="svg-label">${pose.arrow}</text>
    ${exerciseFrame(pose.a, "1 INICIO", 20)}
    <path d="M210 136h48" class="sequence-arrow" marker-end="url(#arrow)"/>
    ${exerciseFrame(midPose(pose.a, pose.b), "2 MOVIMIENTO", 242)}
    <path d="M432 136h48" class="sequence-arrow" marker-end="url(#arrow)"/>
    ${exerciseFrame(pose.b, "3 FINAL", 464)}
  </svg>`;
}

function buildSession(dayIndex) {
  const cardioBlock = [
    ["Cuerda suave 1", 120, "CARDIO 1/10", "Zona comoda | Saltos bajos. Debes poder hablar una frase corta."],
    ["Cuerda ritmo medio 1", 120, "CARDIO 2/10", "Sube el ritmo | Respira fuerte pero controlado. Cambia a marcha rapida si lo necesitas."],
    ["Cuerda fuerte 1", 60, "CARDIO 3/10", "Minuto intenso | Sin llegar al maximo. Prioriza tecnica y aterrizaje suave."],
    ["Cuerda suave 2", 120, "CARDIO 4/10", "Recuperacion activa | Baja pulsaciones sin detenerte por completo."],
    ["Cuerda ritmo medio 2", 120, "CARDIO 5/10", "Ritmo sostenible | Munecas giran, hombros relajados, abdomen firme."],
    ["Cuerda fuerte 2", 60, "CARDIO 6/10", "Minuto intenso | Mantente rapido, pero sin perder coordinacion."],
    ["Cuerda suave 3", 120, "CARDIO 7/10", "Vuelve a controlar | Si hay fatiga, marcha en el sitio con brazos activos."],
    ["Cuerda ritmo medio 3", 120, "CARDIO 8/10", "Ultimo bloque medio | Busca constancia, no velocidad desordenada."],
    ["Cuerda fuerte 3", 60, "CARDIO 9/10", "Ultimo minuto fuerte | Ritmo alto y aterrizaje silencioso."],
    ["Cuerda cierre", 180, "CARDIO 10/10", "Cierre progresivo | 1 min medio, 1 min fuerte, 1 min suave para bajar pulsaciones."]
  ].map(([name, duration, phase, instruction]) => ({ name, duration, instruction, art: "rope", phase }));
  const checkpoint = {
    name: "Continuar o terminar",
    duration: 0,
    instruction: "Ya hiciste el bloque principal de cardio | Si tu esfuerzo percibido es 60% o mas, puedes registrar sesion cumplida. Si estas bien, termina con fuerza corta.",
    art: "rope",
    phase: "CONTROL",
    checkpoint: true
  };
  const finisher = DAYS[dayIndex].exercises.flatMap((name, i) => {
    const [instruction, artType] = INFO[name], target = suggestedTarget(name), round = 1;
    return [
      { name, duration: 8, instruction, art: artType, phase: "ALISTA FUERZA", prep: true, round, target },
      { name, duration: 35, instruction, art: artType, phase: i === 0 ? "HIIT FINAL" : "FUERZA FINAL", work: true, round, target },
      { name: "Registra y respira", duration: 25, instruction: "Guarda tu resultado y toma aire. Poco descanso para mantener intensidad.", art: "rest", phase: "DESCANSO CORTO", rest: true, exerciseName: name, round, target }
    ];
  });
  return [...warmup, ...cardioBlock, checkpoint, ...finisher, { name: "Vuelta a la calma", duration: 120, instruction: "Camina suave, respira lento y estira sin dolor.", art: "mobility", phase: "RECUPERACION" }];
}
...
function renderPreparation() {
  ...
  $("prepareExercises").innerHTML = `<article class="prepare-item"><b>H</b><div><strong>Cardio con cuerda por intervalos</strong><small>3 ciclos: 2 min suave, 2 min medio, 1 min fuerte. Cierre final de 3 min.</small></div><span class="target-pill">18 min</span></article>` + day.exercises.map((name, i) => {
    const last = lastResult(name), target = suggestedTarget(name), unit = unitFor(name);
    return `<article class="prepare-item"><b>${i + 1}</b><div><strong>${name}</strong><small>${last ? `Anterior: ${last} ${unit}` : "Sin registro anterior"} - 35 seg intensos</small></div><span class="target-pill">Meta ${target} ${unit}</span></article>`;
  }).join("");
}
...
function renderHome() {
  ...
  $("weekPlan").innerHTML = DAYS.map((d, i) => {
    ...
    return `<article class="day-card ${i === todayIndex() ? "today" : ""} ${done ? "done" : ""}">
      <div class="day-dot">${done ? "OK" : d.day[0]}</div><div><strong>${d.day} - ${d.title}</strong><small>${d.split} - ${d.focus}</small></div>
...
function recordsFor(names) {
  return names.flatMap(name => repHistory[name] || []).map(r => Number(r.value) || 0).filter(Boolean);
}
function latestWeight() {
  const entries = bodyMetrics.weights || [];
  return entries.length ? entries[entries.length - 1] : null;
}
function weightScore() {
  const entries = bodyMetrics.weights || [];
  if (!entries.length) return 30;
  if (entries.length === 1) return 55;
  const first = entries[0].value, last = latestWeight().value;
  const drift = first ? Math.abs(last - first) / first * 100 : 0;
  return clamp(68 + Math.min(22, entries.length * 4) - Math.max(0, drift - 6) * 3, 35, 100);
}
function weightTrendText() {
  const entries = bodyMetrics.weights || [];
  if (!entries.length) return "Registra tu peso para completar el eje Peso.";
  const last = latestWeight();
  if (entries.length === 1) return `Peso actual: ${last.value} kg. Con otro registro calculo tendencia.`;
  const first = entries[0], diff = +(last.value - first.value).toFixed(1);
  const trend = diff === 0 ? "estable" : diff > 0 ? `+${diff} kg` : `${diff} kg`;
  return `Peso actual: ${last.value} kg | tendencia: ${trend} desde el primer registro.`;
}
function progressProfile(keys) {
  ...
  const strengthValues = recordsFor(["Sentadilla con pesas", "Remo con pesas", "Remo inclinado", "Peso muerto rumano", "Press de hombros", "Curl de biceps"]);
  const enduranceValues = recordsFor(["Cuerda suave", "Rodillas arriba", "Marcha activa", "Burpee adaptado", "Escaladores lentos"]);
  const coreValues = recordsFor(["Plancha", "Dead bug", "Rueda abdominal corta", "Rodilla-codo lateral", "Escaladores lentos"]);
  ...
  const strength = clamp((average(strengthValues) / 18) * 100 + easyCount * 2, 8, 100);
  const endurance = clamp((recentMinutes / 180) * 65 + (average(enduranceValues) / 35) * 35, 8, 100);
  const core = clamp((average(coreValues) / 30) * 100, 8, 100);
  const mobility = clamp(55 + weekDone * 7 - pauses * 4 - hardCount * 2, 12, 100);
  const weight = weightScore();
  const consistency = clamp((weekDone / 6) * 100, 8, 100);
  return [
    { label: "Fuerza", value: Math.round(strength) },
    { label: "Resistencia", value: Math.round(endurance) },
    { label: "Core", value: Math.round(core) },
    { label: "Movilidad", value: Math.round(mobility) },
    { label: "Peso", value: Math.round(weight) },
...
function renderCoachPanel(keys) {
  ...
  $("coachInsight").textContent = `Tu punto fuerte actual es ${top.label.toLowerCase()} (${top.value}). Siguiente foco: ${low.label.toLowerCase()} (${low.value}). ${weightTrendText()}`;
  $("coachTraits").innerHTML = items.map(x => `<span><b>${x.value}</b>${x.label}</span>`).join("");
  const currentWeight = latestWeight();
  $("weightInput").value = currentWeight ? currentWeight.value : "";
...
function saveCurrentReps() {
  ...
  state.repsSaved = true; $("saveRepsBtn").textContent = "Resultado guardado OK";
}
...
function saveWeight() {
  const value = Math.round((Number($("weightInput").value) || 0) * 10) / 10;
  if (!value) return;
  if (!bodyMetrics.weights) bodyMetrics.weights = [];
  const today = dateKey();
  const existing = bodyMetrics.weights.find(x => x.date === today);
  if (existing) existing.value = value;
  else bodyMetrics.weights.push({ date: today, value });
  bodyMetrics.weights = bodyMetrics.weights.slice(-90);
  localStorage.setItem("fuerte-body", JSON.stringify(bodyMetrics));
  renderHome();
}
...
$("soundBtn").onclick = () => { state.sound = !state.sound; $("soundBtn").textContent = `Sonido ${state.sound ? "ON" : "OFF"}`; };
...
$("saveWeightBtn").onclick = saveWeight;
$("weightInput").addEventListener("keydown", event => { if (event.key === "Enter") saveWeight(); });
